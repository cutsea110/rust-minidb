...
//                 v  /  ========>   /                                       v  <============   v
// +------------+-----------------------+----------------------------- .... -----------------------------------+
// |   Header   | Ptr | Ptr | ... | Ptr |                                    |k| v | k |   v   | k |     v     |
// +------------+-----------------------+----------------------------- .... -----------------------------------+
//              |<----- pointers ------>|<--------- free_space ------------->
//              |<---------------- free_space_offset ----------------------->
//  <- header ->|<--------------- body ------------------------------------------------------------------------> Slotted
//
// * insert: Ptr は index 箇所に挿入し、 key,value は左へ伸ばす
//           よって Ptr は index 順だが対応する key,value は挿入順に左に伸びる
// * remove: key,value は消して詰め直し、 Ptr も消して詰め直す
// * resize: key,value は移動コピーし、 Ptr も offset を併せて len を修正
//           もし len = 0 にしてたら、 free_space_offset を刺すように修正
//
